...
import org.apache.pinot.core.operator.query.SelectionOnlyWindowOperator;
...
public class SelectionPlanNode implements PlanNode {
  ...
  public Operator<IntermediateResultsBlock> run() {
    ...
    if (limit > 0) {
      if (_queryContext.hasWindowFunction()) {
        // Selection only
        TransformOperator transformOperator =
            new TransformPlanNode(_indexSegment, _queryContext, expressions, DocIdSetPlanNode.MAX_DOC_PER_CALL).run();
        return new SelectionOnlyWindowOperator(_indexSegment, _queryContext, expressions, transformOperator);
      }
      ...
      // Selection order-by
      if (SelectionOperatorUtils.isAllOrderByColumnsSorted(_indexSegment, orderByExpressions)) {
